...
const TIMEOUT = 150 // only relevant if RUN_SLOWLY === true
const TOTAL_TIMESTEPS = 1000
...
class Map {
  ...
  public runTimestep() {
    ...
    // if every octopus flashed this round
    if(this.octopuses.every( octopusRow => octopusRow.every( octopus => octopus.hasFlashedThisRound ) ) ) {
...
if(RUN_SLOWLY) {
  ...
  function runTimestep() {
    ...
    if(map.allFlashed) console.log(`All octopuses flashed in sync on timestep ${timestep}`)
